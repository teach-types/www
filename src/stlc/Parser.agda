...
mutual
  ...
  -- Expressions where applications are a list

  pExps : Parser (List⁺ Exp)
  pExps (symbol lambda ∷ ts) = ts |> do
    b ← pBind
    _ ← pSymbol arrow
    e ← pExp
    pure (lam b e ∷ [])
  ...
  -- Possibly empty list of expressions

  pRest : Parser (List Exp)
  pRest ts@(ident x       ∷ _) = ts |> List⁺.toList <$> pExps
  pRest ts@(symbol lpar   ∷ _) = ts |> List⁺.toList <$> pExps
  pRest ts@(symbol lambda ∷ _) = ts |> List⁺.toList <$> pExps
  pRest ts = ts |> pure []
