...
correct-aux {n} (add e0 e1) r s acc = trans run0 (trans run1 step)
 where
  -- the code for  add e0 e1  unfolds, by definition of compile-aux, to
  --   compile-aux e0 (compile-aux e1 (ADD acc))
  -- so we are running the code of e0 with the rest as accumulator.
  -- The induction hypothesis on e0 runs it and pushes its value

  run0 : state n (compile-aux e0 (compile-aux e1 (ADD acc))) r s
           =>* state n (compile-aux e1 (ADD acc)) r (eval e0 r :: s)
  run0 = correct-aux e0 r s (compile-aux e1 (ADD acc))

  -- the induction hypothesis on e1, on the stack left by e0

  run1 : state n (compile-aux e1 (ADD acc)) r (eval e0 r :: s)
           =>* state n (ADD acc) r (eval e1 r :: eval e0 r :: s)
  run1 = correct-aux e1 r (eval e0 r :: s) (ADD acc)

  -- one machine step: ADD pops the two values and pushes their sum,
  -- which is  eval (add e0 e1) r  by definition of eval

  step : state n (ADD acc) r (eval e1 r :: eval e0 r :: s)
           =>* state n acc r (eval e0 r + eval e1 r :: s)
  step = oneStep =>ADD
